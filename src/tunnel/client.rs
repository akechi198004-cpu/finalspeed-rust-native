...
pub async fn run(
    ...
    match transport {
        ...
        TransportMode::FakeTcp => run_faketcp(server, secret, map, socks5).await,
    }
}
                        ...
                        Ok(Some(packet)) => {
                            ...
                            match packet.header.packet_type {
                                PacketType::Data => {
                                    ...
                                    let read_key = derive_key(&secret_recv_clone);
                                    let aad = build_aad(&packet.header);
                                    match decrypt_payload(&packet.payload, &read_key, &aad) {
                                        Ok(plaintext) => {
                                            if let Some(session) =
                                                session_mgr_recv.lookup(&conn_id).await
                                            {
                                                ...
                                                use crate::tunnel::session::UnknownState;
                                                match session_mgr_recv.check_unknown(&conn_id).await
                                                {
                                                    UnknownState::RecentlyClosed => {
                                                        tracing::debug!(
                                                            "Dropping late Data packet for recently closed ConnectionId: {}",
                                        ...
                                        Err(e) => {
                                            tracing::warn!("Failed to decrypt Data payload: {}", e);
                                        }
                                    }
                                }
                                PacketType::Ack => {
                ...
                match listener.accept().await {
                    Ok((tcp_stream, peer_addr)) => {
                        if let Err(e) = tcp_stream.set_nodelay(true) {
                            tracing::warn!("Failed to set TCP_NODELAY for local TCP stream: {}", e);
                        }
                        let conn_id = id_gen_clone.next();
                        tracing::info!(
                            "Accepted TCP connection from {} (ConnId: {}), targeting {}",
            ...
            loop {
                match listener.accept().await {
                    Ok((mut tcp_stream, peer_addr)) => {
                        if let Err(e) = tcp_stream.set_nodelay(true) {
                            tracing::warn!(
                                "Failed to set TCP_NODELAY for local SOCKS5 stream: {}",
                                e
                            );
                        }
                        let conn_id = id_gen_clone.next();
